<x-layout>
    ...
    {{-- <article class="py-8 max-w-screen-md border-b border-gray-300">
      <h2 class="mb-1 text-3xl font-semibold tracking-tight text-gray-900">{{ $post['title'] }}</h2>
      <div class="text-base text-gray-500">
        <a href="/authors/{{ $post->author->username }}">{{ $post->author->name }}</a> in <a href="/categories/{{ $post->category->slug }}">{{ $post->category->name }}</a> | {{ $post->created_at->format('d F Y') }}
      </div>
      <p class="my-4 font-light">
        {{ $post['article'] }}
      </p>
      <a href="/posts" class="text-medium text-blue-500 hover:text-blue-700">&laquo; Back To Posts</a>
    </article> --}}
                  ...
                  <address class="flex items-center mb-6 not-italic">
                      <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                          ...
                          <div>
                              ...
                              <p class="text-base text-gray-500 dark:text-gray-400">Dibuat pada {{ $post->created_at->format('d F Y') }}</p>
